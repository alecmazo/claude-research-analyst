...
Cast / voice map (locked):
    Kimi   (host / narrator) → OpenAI 'onyx'   (speaker key: kimi; opus alias)
    Rock   (Grok analyst)    → OpenAI 'fable'
    Claudia (Claude analyst) → OpenAI 'nova'

Script narration is generated by Kimi (Moonshot). Rock + Claudia lines are
written into the script from Grok/Claude research reports.
...
VOICE_MAP: dict[str, str] = {
    # Host / narrator = Kimi (Moonshot). Internal key "kimi"; "opus" kept as
    # alias for older scripts still stamped with the previous host name.
    #   • Kimi    — host/narrator, male  → onyx  (deep moderator gravitas)
    #   • Rock    — Grok analyst, British male → fable (storyteller cadence)
    #   • Claudia — Claude analyst, female → nova  (bright, energetic)
    "kimi":    "onyx",
    "opus":    "onyx",   # legacy alias → same voice as Kimi
    "rock":    "fable",
    "claudia": "nova",
}
...
SPEAKER_SPEED_OFFSET = {
    "kimi":    1.06,   # host/narrator — bump for energy (onyx can sound flat)
    "opus":    1.06,   # legacy alias
    "rock":    1.00,
    "claudia": 1.00,
}
...
VALID_SPEAKERS = {"kimi", "opus", "rock", "claudia"}  # opus = legacy host alias
...
FORMAT_SECTIONS = {
    ...
    "pre_mortem": [
        "cold_open",                # Kimi: hook — "It's 2027. Stock down 60%."
        "scenario_setup",           # Kimi: paints the failed state in detail
        ...
        "verdict",                  # Kimi: highest-conviction failure path + what would falsify the bull case
    ],
    "memo": [
        "cold_open",                # Kimi: ~30 words, hook + ticker
        "executive_summary",        # Kimi: rating, target, 3-sentence thesis
        "business_overview",        # Kimi: what the company does, segments, drivers
        ...
        "recommendation",           # Kimi: action with sizing, time horizon, stop level
    ],
    "catalysts": [
        "cold_open",                # Kimi: "Tonight: 12 months of catalysts, ranked"
        "setup_quarter",            # Kimi: where we are now
        ...
        "ranking",                  # Kimi + both: rank by importance × probability
        "verdict",                  # Kimi: which catalyst is the actual trade
    ],
    "quick_hit": [
        "cold_open",                # Kimi: 1 turn, ~30 words
        "company_one_breath",       # Kimi: 1 turn, ~60 words
        "rock_take",                # Rock: 1 turn, ~80 words
        "claudia_take",             # Claudia: 1 turn, ~80 words
        "single_round",             # mixed: 3-4 turn debate
        "verdict",                  # Kimi: 1 turn, ~80 words
    ],
...
CAST (the three speakers, never invent a fourth):
  • Kimi    — HOST / NARRATOR. MALE. Sharp, even-keeled, calls bullshit on either side. \
...
IMPORTANT: refer to the host ONLY as "Kimi" — never invent a last name. \
He has no surname in the show. "I'm Kimi" / "Kimi here" / never "Kimi [anything]". \
In JSON, speaker field MUST be "kimi" (not "opus"). Powered by Kimi (Moonshot).
...
Powered by Grok (his research report is the source).
...
Powered by Claude (her research report is the source).
...
Kimi chimes in mid-round, sometimes one analyst gets 2 turns in a row \
...
COLD OPEN VARIETY — applies to every format
  ...
  every episode with "Welcome back to the DGA HiTech Podcast, I'm Kimi, and
...
def _format_addendum(format: str) -> str:
  ...
  1. cold_open            — Kimi only, ~35–45 words. ONE-LINE hook + ticker.
  2. company_in_60        — Kimi only, ~120–150 words. Plain-English what the company does.
  ...
  5. round_thesis         — 4–6 turns, mostly Rock + Claudia, Kimi 1–2 follow-ups.
  ...
  9. verdict              — Kimi only, 1–2 turns, ~110–140 words. Must say "the more convincing pitch tonight was Rock" or "...was Claudia" with 2 specific reasons from THE DEBATE. No ties."""
  ...
  1. cold_open            — Kimi only, ~35–50 words. Hook MUST establish: "It's [year + 2]. {TICKER} is down [60-80]%. What killed it?"
  2. scenario_setup       — Kimi only, ~130–160 words. Paint the failed state in vivid detail: stock price, what the news headlines look like, who got fired, what the bulls now say in hindsight.
  ...
  8. verdict              — Kimi only, 1–2 turns, ~120–150 words. Names the HIGHEST-CONVICTION failure path + one falsification test ("if X happens within 6 months, the bears are wrong")."""
        winner_field = '  "winner": "rock" | "claudia",  // whoever Kimi names as having the most-likely failure thesis\n'

    elif format == "memo":
        structure = """STRUCTURE — Investment Memo Walk-Through. Treat this like a real IC presentation: structured, sober, conclusive.

  1. cold_open            — Kimi only, ~30–45 words. Crisp hook + ticker + rating preview.
  2. executive_summary    — Kimi only, ~130–160 words. Rating, 12-mo target, 3-sentence thesis, sizing recommendation.
  3. business_overview    — Kimi only, ~160–200 words. Segments, key drivers, customer base, geographic mix.
  ...
  8. recommendation       — Kimi only, 1–2 turns, ~140–170 words. Action with: SIZING (% of book), TIME HORIZON, STOP-LOSS or hedge structure. End with "the committee recommends..." or "this analyst recommends..."

This format does NOT use a 'winner' field — it ends with a recommendation, not a debate result."""
        winner_field = '  "winner": "rock" | "claudia",  // for memo format, set to whoever Kimi\'s recommendation aligns with directionally\n'
  ...
  1. cold_open            — Kimi only, ~30–45 words. "Tonight: 12 months of catalysts, ranked." + ticker.
  2. setup_quarter        — Kimi only, ~80–110 words. Where the stock is now. Recent earnings, momentum, sentiment one-liner.
  ...
  8. ranking              — 3–5 turns. Both speakers + Kimi assemble a ranked list of the 7 events by IMPORTANCE × PROBABILITY. Explicit ranking, no hedging.
  9. verdict              — Kimi only, 1–2 turns, ~100–130 words. Which SINGLE catalyst is the actual trade. Specific entry timing.
...
Use real catalyst dates from the source reports where possible. If a date isn't stated, use "~Q2" / "late spring" — DON'T invent specific dates."""
        winner_field = '  "winner": "rock" | "claudia",  // for catalysts format, set to whoever Kimi\'s "the trade" recommendation aligns with\n'
  ...
  1. cold_open            — Kimi only, 1 turn, ~25–35 words. ONE punchy hook.
  2. company_one_breath   — Kimi only, 1 turn, ~50–70 words. The company explained in 60 sec.
  ...
  6. verdict              — Kimi only, 1 turn, ~70–100 words. Calls the winner with one tight reason.
...
You MUST produce sections in this order (using actual section IDs as listed):
  ...
  1. cold_open                — Kimi only, ~35–50 words. "Tonight: 3 names — [tickers]. Quick takes, then a wrap."
  2. For EACH ticker in order, produce 3 sections:
     {N}_intro       — Kimi only, ~30–50 words. Sets up that ticker's story in one paragraph.
...
speakers to specific sections (e.g. "Kimi only" or "Rock only" or "mixed").

  • Kimi    — leads narrative sections (cold_open, executive_summary,
...
Now write the episode. Remember:
  ...
  • Kimi names a winner with 2 specific reasons drawn from THE DEBATE
  • "winner" field at the top must match Kimi's verdict
...
def validate_script(script: dict[str, Any]) -> dict[str, Any]:
    ...
    for sec in sections:
        ...
        if sid in opus_only_ids:
            non_opus = [t for t in turns
                        if (t.get("speaker") or "").lower() not in ("opus", "kimi")]
            if non_opus:
                errors.append(f"{sid} should be Kimi only — found {len(non_opus)} other speakers")
...
def generate_script(
    ...
    *,
    ...
    # Narration / script authored by Kimi (Moonshot) by default — host voice.
    # Rock + Claudia lines still speak Grok/Claude research. Override via model=
    # or Settings podcast_script route (claude/grok fallbacks).
    _route = "kimi"
    try:
        if hasattr(_ca, "get_task_route"):
            _route = _ca.get_task_route("podcast_script") or "kimi"
    except Exception:
        _route = "kimi"
    if model and "claude" in str(model).lower():
        _route = "claude"
    elif model and "grok" in str(model).lower():
        _route = "grok"

    print(f"🎙️ [podcast/script {ticker}] calling {_route} for narration  "
          f"sys={len(system):,}ch  user={len(user):,}ch", flush=True)
    _tc = _t.time()
    try:
        raw = _ca.call_llm(
            _route,
        ...
        if _route == "kimi":
            print(f"⚠️  [podcast/script {ticker}] Kimi failed ({e!s:.160}); falling back to Claude…", flush=True)
            try:
                raw = _ca.call_claude(
                    system_prompt=system,
                    user_content=user,
                    model=_ca.CLAUDE_MODEL,
                    usage_capture=_track_cost,
                    should_cancel=should_cancel,
                )
                _route = "claude"
                ...
                print(f"❌ [podcast/script {ticker}] Claude fallback failed: {e2!s:.500}", flush=True)
                raise
        else:
            print(f"❌ [podcast/script {ticker}] {_route} call failed: {e!s:.500}", flush=True)
            raise
    print(f"🎙️ [podcast/script {ticker}] {_route} returned {len(raw):,}ch ({_t.time()-_tc:.1f}s)", flush=True)
    ...
    script["narrator_engine"] = _route  # kimi | claude | grok that wrote the script

    # Normalize host speaker: legacy "opus" → "kimi"
    try:
        for sec in (script.get("sections") or []):
            for turn in (sec.get("turns") or []):
                if (turn.get("speaker") or "").lower() == "opus":
                    turn["speaker"] = "kimi"
    except Exception:
        pass
...
def _tts_turn(client, speaker: str, text: str, intensity: str,
    ...
    sp_l = {
        "alec": "kimi", "alex": "kimi", "opus": "kimi", "host": "kimi",
        "narrator": "kimi", "claude": "claudia",
    }.get(sp_l, sp_l)
...
def synthesize_episode(
    ...
    *,
    ...
    for i, (sid, turn) in enumerate(flat, start=1):
        sp = (turn.get("speaker") or "kimi").lower()
        if sp == "opus":
            sp = "kimi"
        text = (turn.get("text") or "").strip()
