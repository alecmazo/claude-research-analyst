...
def draft_cold_email(
    *,
    ...
    subject = f"{company} × Edyta Śliwińska (DWTS) — team experience worth remembering"
    body = f"""{greeting},
...
"""
    return {"subject": subject, "body": body.strip() + "\n", "to_name": contact_name, "to_title": contact_title}
...
def save_outreach_draft(
    *,
    ...
    path = OUTREACH_DIR / f"{safe}_{sequence_step}_{stamp}.json"
    payload = {
...
def write_edyta_brief(
    *,
    ...
    safe = "".join(c if c.isalnum() or c in "-_" else "_" for c in company)[:40]
    path = BRIEFS_DIR / f"EDYTA_BRIEF_{safe}.md"
...
"""
    ...
    update_prospect(prospect["id"], edyta_brief_path=str(path))
    return path
